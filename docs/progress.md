...
- 단계: 퀴즈 추가 저장 실패 시 메모리 원상 복구 구현, 사용자 직접 확인 대기
- 현재 통합 브랜치: `main`
...
- 퀴즈 추가 저장 실패 처리와 관련 문서 갱신은 현재 미커밋이다.
...
- 정상 저장에서는 새 퀴즈가 메모리와 확인용 JSON에 모두 남는지 확인한다.
- 저장 실패를 주입한 검사에서는 새 퀴즈가 메모리 목록에서 제거되고 취소 안내가
  출력되는지 확인한다.
