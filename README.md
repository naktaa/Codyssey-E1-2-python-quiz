...
> 현재 상태: 필수 기능과 JSON 상태 관리를 완료했고, `feature/bonus`에서
> 문제 수 선택·무작위 출제·수동 힌트·누적 점수·퀴즈 삭제·최근 플레이
> 기록을 구현했다.
...
| 퀴즈 풀기 | 카테고리 선택, 무작위 출제, 정오답·누적 점수 출력 | 직접 실행 확인 |
...
| 퀴즈 삭제 | 카테고리·문제 번호 선택 후 `y/n` 확인과 즉시 저장 | 직접 실행 확인 |
...
- 결과 화면에 정답 수, 힌트 사용 횟수와 `획득 점수/만점`을 함께 표시한다.
- 최고 점수는 카테고리별 누적 점수의 최댓값을 저장한다. 선택한 문제 수가
  달라도 원점수를 그대로 비교하는 단순한 방식이며, 평가 때 이 기준을 설명한다.
...
- Git: 미션 산출물이므로 추적
- 저장 시점: 퀴즈 추가·삭제 직후, 최고 점수 갱신 직후, 정상·중단 종료 시
