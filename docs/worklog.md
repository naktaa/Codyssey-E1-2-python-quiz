...
- 최종 게임 진행 클래스명은 `GameManager`다. 이전 작업 기록의 `QuizGame`은
  당시 코드와 실행 명령을 그대로 보존한 역사적 기록이다.
...
## 2026-08-10 — 최종 제출 기준 확정과 문서 동기화

- 환경: Linux / bash / Python 3.12.3
- 브랜치: `main`
- 기준 커밋:
  `934811e Refactor: 메뉴·점수 처리와 상태 저장 형식 개선`
- 목표: 사용자가 확정한 클래스명과 증거 채택 방식을 현재 문서 전체에
  일관되게 반영한다.

### 최종 결정

- 게임 진행 클래스는 현재 코드와 동일하게 `GameManager`로 통일한다.
- 2026-08-09 macOS 직접 실행 기록과 2026-08-10 최종 코드 회귀 검사를
  함께 메뉴·플레이·점수·저장 증거로 채택한다.
- 기존 `evidence/screenshots/git-log.png`를 브랜치 분기·병합 증거로 채택하고
  새 Git 그래프는 추가하지 않는다.
- 이전 직접 실행 로그와 Git 이력은 실제 증거이므로 내용을 재구성하지 않는다.

### 최종 점검

임시 디렉터리와 입력 대역을 사용한 Python 격리 회귀 검사에서 다음을 확인했다.

- Python 소스 구문과 import
- 기본 4지선다 퀴즈 5개
- 파일 없음 → 기본 상태 생성 → 저장·재로드
- 추가 퀴즈 저장 후 새 `GameManager`에서 재로드
- 손상 JSON 백업과 기본 상태 복구
- 빈 입력·문자·범위 밖 숫자의 공통 검증
- 저장 실패 시 퀴즈 추가·삭제와 플레이 기록·최고 점수 롤백
- `EOFError`와 `KeyboardInterrupt`의 traceback 없는 안전 종료
- README와 문서·증거의 로컬 Markdown 링크
- Git 커밋 56개, 병합 커밋 4개, `main`·`origin/main` 동기화
- 추적 텍스트의 토큰·키·비밀번호 형식과 개인 절대 경로 미검출

### 문서 변경

- `MISSION.md`, `AGENTS.md`: 필수 클래스명을 `Quiz`·`GameManager`로 통일
- `README.md`: 직접 실행과 회귀 검사의 최종 증거 역할 명시
- `docs/requirements.md`: 영속성·최종 기능·Git 그래프 항목을 확정 상태로 갱신
- `docs/progress.md`: 최종 커밋·증거 채택·최종 판정을 현재 상태로 갱신
- `evidence/logs/`: 원본 출력은 유지하고 두 기록을 함께 채택하는 현재성 안내만 갱신

### Git 상태와 다음 작업

- 점검 시작 시 `main`과 `origin/main`은 `934811e`에서 일치했다.
- 이 문서 동기화 변경은 미커밋 상태다.
- 다음 작업: 사용자가 원할 때 최종 문서 변경을 commit·push한다.

---

## 2026-08-10 — 메뉴 디스패치와 실시간 점수·상태 파일 형식 개선

- 환경: Linux / bash / Python 3.12.3
- 브랜치: `main`
- 기준 커밋: `1716e71 Docs: 최종 실행과 Git 증거 정리`
- 목표: 메뉴 분기를 딕셔너리 디스패치로 단순화하고, 문제별 점수를 즉시 반영해
  현재 누적 점수를 표시하며 상태 파일의 가독성과 백업 이름을 정리한다.

### 변경 내용

- `GameManager.run()`의 메뉴 1~6을 호출 가능한 메서드 딕셔너리로 연결하고 기존
  `if/elif` 실행 분기를 제거했다. 종료 메서드 실행 후 반복 종료를 위한 조건만
  유지했다.
- 정답이면 현재 점수에 3점을 더하고 해당 정답에서 힌트를 봤으면 2점을 차감한다.
  오답과 시간 초과에는 차감을 적용하지 않는다.
- 정답·오답·시간 초과 안내 뒤에 `현재 점수: N점`을 공통 출력한다.
- 손상 백업 파일명에서 마이크로초를 제거하고 초 단위 이름이 충돌하면 `-1`, `-2`
  순번을 붙여 기존 백업을 보존한다.
- JSON 저장 들여쓰기를 2칸에서 4칸으로 변경하고 추적 중인 `state.json`도 한글을
  유지한 4칸 형식으로 정리했다.
- README, 아키텍처, 진행 상태, 요구사항, 코드 학습 가이드와 기존 실행 증거의
  현재성 안내를 최신 코드에 맞췄다.

### 실행 검증

격리된 메모리 상태 관리자와 제한시간 입력 대역을 사용하는 Python 회귀 스크립트를
실행했다.
...
- 힌트 후 오답: `현재 점수: 0점`
- 힌트 후 시간 초과: `현재 점수: 0점`
- 문제별 `점을 획득했습니다` 문구가 더 이상 출력되지 않음
- 메뉴 2가 목록 메서드, 메뉴 6이 안전 종료 메서드를 호출하고 반복 종료
- 같은 초의 두 백업이 기본 이름과 `-1` 순번으로 각각 보존
- UTF-8 한글과 JSON 4칸 들여쓰기 저장
- Python 소스 6개 구문 검사 통과
...
- JSON 문법과 Git 공백 검사가 통과했다.
- 사용자 macOS 터미널의 최신 `현재 점수` 화면은 직접 재확인이 필요하다.

### 증거

- `evidence/logs/current-score-regression.md`: 문제별 3점·1점·0점·0점과 메뉴·백업·
  JSON 회귀 검사 결과
...
- commit: 미커밋
- push: 미실시
- 다음 작업: `python3 main.py --test`로 최신 플레이 화면을 직접 확인하고 원본
  로그를 확보한다.
...
- 목표: 코드, Git 이력과 현재 설명 문서의 구현·검증 상태를 같은 기준으로 맞춘다.

### 확인과 변경 내용

- 점검 시작 시 작업 트리는 깨끗했고 `main`, `origin/main`은 `9d76c98`에서
  일치했다.
- 기능 코드, README, 아키텍처와 코드 학습 가이드는 10초 제한 시간, 5초 힌트,
  3점에서 2점을 차감하는 점수와 `GameManager` 구조로 일치했다.
- `docs/progress.md`의 이전 기준 커밋과 `commit 대기` 상태를 실제 commit·push 및
  로컬 TTY 검증 완료 상태로 갱신했다.
- `docs/requirements.md`의 자동 힌트·제한 시간 항목을 `9d76c98`과 연결하고
  `실행 검증 완료, 최신 증거 필요`로 구분했다.
- 최신 기능 작업 기록의 미커밋·push 미실시 문구를 실제 Git 상태로 바로잡았다.
- 과거 작업 당시의 `QuizGame`, 20초·10초 기록과 기존 화면은 실제 개발 이력 및
  개선 전 증거이므로 변경하지 않았다.

### 검증

```zsh
/usr/local/bin/python3.12 -m py_compile main.py src/*.py
/usr/local/bin/python3.12 -m json.tool state.json >/dev/null
git diff --check
```

- Python 구문, 추적 JSON 형식과 문서 공백 검사가 통과했다.
- 현재 기능 설명에서 이전 점수 상수, 20초 제한 시간이나 10초 힌트를 현재 규칙으로
  잘못 설명하는 문구는 발견되지 않았다.
- 사용자 직접 실행 화면, 영속성, 최종 Git 그래프와 clone·pull 증거는 남은
  최종 작업으로 유지했다.

### Git 상태와 다음 작업

- 이번 문서 갱신: 미커밋
- 다음 작업: 사용자가 현재 기능을 직접 확인하고 최신 실행 화면을 확보한다.

---

## 2026-08-09 — 게임 화면과 힌트 점수 차감 규칙 개선

- 환경: macOS / zsh / Python 3.12.13
- 브랜치: `main`
- 기준 커밋: `e22a61a ㅁ`
- 목표: 시연 화면의 흐름을 명확하게 하고 힌트 점수를 차감 방식으로 표현하며
  문제당 진행 시간을 줄인다.

### 변경 내용

- 메뉴와 각 기능에 같은 구분선을 사용하고 등록 문제 수와 최고 점수를 메뉴에
  표시했다.
- 플레이 시작 화면에 선택 문제 수, 10초 제한 시간과 5초 힌트를 표시했다.
- 각 문제에 `[문제 현재/전체]` 진행 수를 표시하고 최종 결과를 별도 구역으로
  구분했다.
- 정답 기본 점수 3점에서 힌트 사용 시 2점을 차감해 1점을 얻도록 계산 구조를
  변경했다. 화면에는 문제별 획득 점수와 최종 점수만 표시한다.
- 결과의 `자동 힌트 공개` 문구를 `힌트 사용 횟수`로 변경했다.
- 문제 제한 시간을 20초에서 10초, 자동 힌트 공개를 10초에서 5초로 변경했다.
- `GameManager` 이름은 사용자의 구조 통일 의도에 따라 유지하고 코드 학습 가이드의
  이전 `QuizGame` 표기를 현재 이름으로 바로잡았다.
- README, 아키텍처, 진행 상태, 요구사항과 코드 학습 가이드의 현재 규칙을
  10초·5초와 힌트 차감 방식으로 갱신했다. 과거 작업 당시의 20초·10초 로그는
  실제 이력으로 유지했다.
...
/usr/local/bin/python3.12 -m py_compile main.py src/*.py
/usr/local/bin/python3.12 -m json.tool state.json >/dev/null
printf '6\n' | /usr/local/bin/python3.12 main.py --test
git diff --check
```

- Python 구문, 추적 JSON 형식과 Git 공백 검사가 통과했다.
- 테스트 상태에서 새 메뉴 구분, 등록 문제 수, 최고 점수와 메뉴 1~6 출력을
  확인하고 정상 종료했다.
- 임시 상태와 고정된 4문제 결과를 사용한 격리 검사에서 다음을 확인했다.
  - 힌트 전 정답 3점
  - 힌트 후 정답 `3점 - 2점 = 1점`
  - 오답과 시간 초과 0점
  - `힌트 사용 횟수: 2회`, `최종 점수: 4점`
  - 최고 점수와 플레이 기록에 최종 4점 저장
- `TimedTerminalInput` 기본값이 제한 시간 10초, 힌트 공개 5초인지 확인했다.
- 실제 TTY 검사에서 10초부터 카운트다운하고 5초 후 힌트 줄을 제자리에서
  갱신한 뒤, 시간 초과 결과와 메뉴 복귀 및 Ctrl+C 안전 종료를 확인했다.
- 실제 macOS TTY의 1초 카운트다운과 제자리 힌트 갱신은 사용자 직접 확인이
  필요하다.

### Git 상태와 다음 작업

- commit: `9d76c98 Fix: ui 일부 개선 및 힌트 점수 계산 개선`
- push: 완료, `main`과 `origin/main`이 `9d76c98`에서 일치
- 다음 작업: 사용자가 `python3 main.py --test`로 메뉴, 10초 카운트다운,
  5초 힌트와 3점·1점 결과를 직접 확인하고 현재 화면을 확보한다.

---

## 2026-08-09 — 관리자 이름과 기본 퀴즈 생성 시점 정리

- 환경: Linux / bash / Python 3.12.3
- 브랜치: `main`
- 기준 커밋: `2e52e63 Refactor: 구조 단순화 및 불필요 기능 제거`
- 목표: 게임 진행 클래스 이름을 `GameManager`로 통일하고 기본 퀴즈를 필요한
  시점에만 생성한다.

### 변경 내용

- `QuizGame` 클래스 이름을 파일명과 변수명에 맞춰 `GameManager`로 변경했다.
- `StateManager` 생성자는 상태 파일 경로만 전달받는다.
- `main.py`에서 기본 퀴즈를 미리 생성하고 전달하는 코드를 제거했다.
- `_create_default_state()`는 파일 없음·손상·읽기 실패 시점에 새 기본 퀴즈를 만든다.
- `StateManager`는 별도의 기본 퀴즈 목록을 계속 보관하지 않는다.

### 실행 검증

- Python 파일 7개의 구문 검사를 통과했다.
- 정상 `state.json` 로드에서는 기본 퀴즈 생성 함수가 호출되지 않았다.
- 임시 경로의 파일 없음과 손상 파일 복구에서만 기본 퀴즈가 한 번씩 생성됐다.
- `GameManager`로 실제 상태와 `--test` 상태를 정상적으로 불러왔다.
- 손상 파일 백업과 기본 상태 복구가 유지되는 것을 확인했다.
