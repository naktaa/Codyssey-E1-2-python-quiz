...
- 단계: 카테고리 제거와 직접 입출력 구조 단순화 구현, 사용자 직접 확인 대기
- 현재 통합 브랜치: `main`
...
- 카테고리 제거와 직접 입출력 구조 단순화는 현재 미커밋이다.
...
- 카테고리 선택 없이 전체 문제 수 선택과 무작위 플레이가 시작되는지 확인한다.
- 추가·목록·삭제에서 카테고리 입력이나 제목이 더 이상 나타나지 않는지 확인한다.
- 메뉴 3번에서 단일 최고 점수와 카테고리 없는 최근 기록이 표시되는지 확인한다.
- 기존 카테고리형 확인용 JSON을 불러온 뒤 저장하면 새 스키마로 변환되는지 확인한다.
- 확인 결과에 따라 문서를 보완한 뒤 사용자가 commit·push한다.
