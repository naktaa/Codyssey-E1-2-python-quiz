...
> 현재 상태: 필수 기능과 보너스 기능의 `main` 병합을 완료하고, 테스트 기록 정리와
> 점수·히스토리 표시 및 JSON 스키마 정합성 개선까지 반영했다. 다음 작업에서는
> 힌트를 문제별 JSON 저장 방식으로 변경한다.
...
| 수동 힌트 | 정답과 오답 하나를 후보로 표시하고 사용 횟수 집계 | 구현 완료, 직접 확인 필요 |
...
- `show_hint()`: 정답 선택지와 무작위 오답 선택지 하나를 두 개의 후보로 출력한다.
