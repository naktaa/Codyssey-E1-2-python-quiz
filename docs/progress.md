...
- 단계: 보너스 퀴즈 삭제와 JSON 즉시 반영 구현 완료
- 현재 통합 브랜치: `main`
...
날짜와 시간을 포함하는 점수 기록 히스토리의 JSON 구조와 표시 방식을 설계한다.

- 최고 점수와 별도로 매 플레이 결과를 저장할 필드를 정한다.
- 카테고리, 점수, 만점, 정답 수, 힌트 사용 횟수와 플레이 시각의 저장 범위를
  결정한다.
- 기존 `state.json`과의 하위 호환 및 손상 검증 기준을 먼저 설명한 뒤 구현한다.
