...
중복 없이 무작위로 출제된다. 각 문제의 제한 시간은 20초이며 10초가 지나면 힌트가
자동으로 공개된다.
...
게임 상태 파일을 담당한다. `QuizGame`에서 퀴즈·최고 점수·플레이 기록을 전달받아
...
20초 제한 시간, 1초 카운트다운, 10초 후 자동 힌트와 시간 초과 입력 정리를 처리한다.
...
| 메뉴 | 수행 기능 | 상태 파일 처리 |
|---|---|---|
| 1. 퀴즈 풀기 | 문제 수 선택, 무작위 출제, 제한 시간 답 입력과 점수 계산 | 결과와 최고 점수 저장 |
...
JSON은 UTF-8, 한글 유지, 들여쓰기 2칸 형식으로 저장한다.
...
- 손상 파일: 원본을 별도 파일로 백업한 뒤 기본 상태로 복구한다.
- 백업 실패: 손상 원본을 보호하기 위해 해당 실행의 저장을 중지한다.

상태 변경 시 임시 JSON 파일을 먼저 작성한 뒤 실제 상태 파일로 교체한다. 퀴즈
추가·삭제 또는 플레이 기록 저장이 실패하면 메모리 변경도 이전 상태로 되돌린다.
...
- [ ] 각 문제에서 20초 카운트다운이 동작한다.
- [ ] 10초 전 정답은 3점, 자동 힌트 후 정답은 1점으로 계산된다.
