...
- 단계: 테스트 기록 정리와 점수·히스토리 표시 개선 완료, 다음 코드 수정 대기
- 현재 통합 브랜치: `main`
...
- `Quiz`에 `hint` 속성을 추가한다.
- 기본 퀴즈와 `state.json`의 각 문제에 직접 작성한 힌트 문장을 저장한다.
- `to_dict()`·`from_dict()`와 상태 검증에 `hint`를 반영한다.
- 새 퀴즈를 추가할 때 힌트도 함께 입력받는다.
- 현재의 정답과 무작위 오답을 두 후보로 보여 주는 `show_hint()`는 저장된 힌트
  문장을 출력하도록 변경한다.
- 기존 JSON에 `hint`가 없을 때의 호환 방식을 구현 전에 확정한다.

### 2. 퀴즈 추가 저장 실패 시 메모리 상태 처리 결정
