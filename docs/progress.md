...
- 단계: JSON 기능 `main` 병합과 기본 상식 퀴즈 5개 구현 완료
- 현재 통합 브랜치: `main`
- JSON 병합: `4984e31 Merge: JSON 상태 관리 브랜치 병합`
- Git checkout·commit·push 등 직접 조작은 이후 사용자가 수행
...
보너스 기능의 기반이 되는 풀이 문제 수 선택을 구현한다.

- 선택한 카테고리의 전체 문제 수를 안내한다.
- 사용자가 1부터 전체 문제 수 사이에서 풀이 개수를 선택한다.
- 선택한 개수만큼 출제하고 정답 수와 점수를 해당 개수 기준으로 계산한다.
- 문제 무작위 출제는 다음 단계에서 이 선택 결과에 연결한다.
