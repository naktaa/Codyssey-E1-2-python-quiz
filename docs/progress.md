...
- 단계: 문제별 JSON 저장 힌트 구현 완료, 사용자 직접 실행 확인 대기
- 현재 통합 브랜치: `main`
...
- 메뉴 3번은 최고 점수와 시각 기준 최근 플레이 기록 5개의 획득 점수를 함께
  표시한다.
- 새 카테고리를 포함해 문제별 힌트가 있는 퀴즈를 자유롭게 추가할 수 있다.
...
- 추적 중인 `state.json`에 빈 `score_history` 목록을 반영했다.
...
- 문제별 힌트 저장 방식 구현과 문서 갱신은 현재 미커밋이다.
...
- 사용자가 `QUIZ_STATE_MODE=test python3 main.py`로 직접 실행한다.
- 기본 문제의 저장된 힌트 출력과 힌트 사용 정답 1점을 확인한다.
- 새 퀴즈에 힌트를 입력하고 재실행 후 같은 힌트가 유지되는지 확인한다.
- 확인 결과에 따라 문서를 보완한 뒤 사용자가 commit·push한다.

## 이후 작업

- 현재는 새 퀴즈를 메모리 목록에 먼저 추가하고 JSON 저장을 시도한다.
- 저장 실패 후에도 같은 실행에서는 새 퀴즈를 목록과 플레이에서 사용할 수 있지만,
  종료 시 재저장도 실패하면 다음 실행에서 사라진다.
- 저장 실패 시 방금 추가한 퀴즈를 목록에서 제거해 파일과 메모리를 일치시킬지,
  현재처럼 메모리에 유지하고 종료 때 다시 저장할지 결정한 뒤 수정한다.
